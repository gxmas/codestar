...
--   Returns 'Nothing'.
handleMessage :: (MonadCatch m, MonadIO m) => Server m -> ServerConfig -> Message -> m (Maybe Response)
...
handleBatch :: (MonadCatch m, MonadIO m) => Server m -> ServerConfig -> [BatchElement] -> m (Maybe [Response])
...
--   Nothing       -> pure ()
-- @
handleRaw :: (MonadCatch m, MonadIO m) => Server m -> ServerConfig -> ByteString -> m (Maybe ByteString)
