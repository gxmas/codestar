...
-}
withRetry :: RecoveryEngine -> LlmClientDict -> LlmClientDict
withRetry engine client =
  client
 ...
 where
  recover opName action =
    withRecoveryEither engine opName isTransient retryDelay action
