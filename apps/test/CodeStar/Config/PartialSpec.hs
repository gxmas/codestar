...
-- Count how many of the 9 top-level Last fields are populated.
countSetFieldsPC :: PartialConfig -> Int
countSetFieldsPC p = length . filter id $
  [ isSet p.provider, isSet p.modelRoles, isSet p.planningMode
  , isSet p.sandboxMode, isSet p.workspacePath
...
fieldBandPC p
  | n == 0    = "empty (0/9 fields)"
  | n <= 3    = "sparse (1-3/9 fields)"
  | n <= 7    = "mixed (4-7/9 fields)"
  | otherwise = "rich (8-10/9 fields)"
...
spec = describe "PartialConfig monoid laws" $ do
  ...
  prop "left identity: mempty <> p == p" $
    \p ->
      classify (fieldBandPC p == "empty (0/9 fields)") "empty" $
      classify (fieldBandPC p == "sparse (1-3/9 fields)") "sparse (1-3)" $
      classify (fieldBandPC p == "mixed (4-7/9 fields)") "mixed (4-7)" $
      classify (fieldBandPC p == "rich (8-10/9 fields)") "rich (8-10)" $
